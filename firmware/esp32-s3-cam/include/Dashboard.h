...
function draw(name,color,unit,decimals,end,dt){
  ...
  c.fillStyle='#738592';c.font=(10*scale)+'px monospace';c.textBaseline='top';c.fillText(hi.toFixed(decimals),5*scale,4*scale);c.textBaseline='bottom';c.fillText(lo.toFixed(decimals),5*scale,h-4*scale);c.textAlign='right';c.fillText('p-p '+peakToPeak.toFixed(decimals),w-5*scale,h-4*scale);c.textAlign='left';
