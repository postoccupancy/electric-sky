...
function parsePacket(buffer){
  ...
  bmeDetail.textContent='queue '+d.getUint16(46,true)+' · overruns '+d.getUint32(28,true);powerDetail.textContent='queue '+d.getUint16(48,true)+' · overruns '+d.getUint32(32,true);audioDetail.textContent='queue '+d.getUint16(50,true)+' · overruns '+d.getUint32(36,true);transportQueued=d.getUint16(52,true);transportDropped=d.getUint32(54,true);scheduledHz=d.getUint16(58,true)/10;firmwareUptimeMs=d.getUint32(60,true);if(baselineDrops===null){baselineDrops=transportDropped;baselineUptimeMs=firmwareUptimeMs}
...
function draw(name,color,unit,decimals,end,dt){
  ...
  let lo=Infinity,hi=-Infinity;ring.visitRange(start,end,s=>{if(s.v<lo)lo=s.v;if(s.v>hi)hi=s.v;const b=Math.min(bins-1,Math.max(0,Math.floor((s.t-start)/WINDOW_US*bins)));if(s.v<mins[b])mins[b]=s.v;if(s.v>maxs[b])maxs[b]=s.v});if(lo===Infinity)return;let range=hi-lo;if(range<1e-9)range=1;const wantedLo=lo-range*.12,wantedHi=hi+range*.12;if(canvas._axisLo===undefined){canvas._axisLo=wantedLo;canvas._axisHi=wantedHi}else{if(wantedLo<canvas._axisLo)canvas._axisLo=wantedLo;else canvas._axisLo+=(wantedLo-canvas._axisLo)*Math.min(1,dt/60000);if(wantedHi>canvas._axisHi)canvas._axisHi=wantedHi;else canvas._axisHi+=(wantedHi-canvas._axisHi)*Math.min(1,dt/60000)}lo=canvas._axisLo;hi=canvas._axisHi;
