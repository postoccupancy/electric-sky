...
function draw(name,color,unit,decimals,end,dt){
  ...
  let lo=Infinity,hi=-Infinity;ring.visitRange(start,end,s=>{if(s.v<lo)lo=s.v;if(s.v>hi)hi=s.v;const b=Math.min(bins-1,Math.max(0,Math.floor((s.t-start)/WINDOW_US*bins)));if(s.v<mins[b])mins[b]=s.v;if(s.v>maxs[b])maxs[b]=s.v});if(lo===Infinity)return;const peakToPeak=hi-lo;let range=peakToPeak;if(range<1e-9)range=1;const wantedLo=lo-range*.12,wantedHi=hi+range*.12;if(canvas._axisLo===undefined){canvas._axisLo=wantedLo;canvas._axisHi=wantedHi;canvas._axisHoldMs=30000}else{let expanded=false;if(wantedLo<canvas._axisLo){canvas._axisLo=wantedLo;expanded=true}if(wantedHi>canvas._axisHi){canvas._axisHi=wantedHi;expanded=true}if(expanded)canvas._axisHoldMs=30000;else if(canvas._axisHoldMs>0)canvas._axisHoldMs=Math.max(0,canvas._axisHoldMs-dt);else{canvas._axisLo+=(wantedLo-canvas._axisLo)*Math.min(1,dt/60000);canvas._axisHi+=(wantedHi-canvas._axisHi)*Math.min(1,dt/60000)}}lo=canvas._axisLo;hi=canvas._axisHi;
