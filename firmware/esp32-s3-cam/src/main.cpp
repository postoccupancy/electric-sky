...
body{margin:0;background:#080a0d;color:#dce6ee;font:14px monospace}header{padding:12px 18px;border-bottom:1px solid #26313a;display:flex;justify-content:space-between;gap:16px;align-items:center}h1{margin:0;color:#adf;font-size:16px;letter-spacing:.12em}main{padding:16px;text-align:center}img{display:block;max-width:100%;height:auto;margin:auto;border:1px solid #26313a;image-rendering:auto}button,select{background:#17212a;color:#adf;border:1px solid #33424e;padding:6px 9px;font:12px monospace}#state{color:#8ba0af}#activity{display:inline-block;width:6px;height:6px;margin-right:8px;border-radius:50%;background:#45515b;vertical-align:middle}#activity.busy{background:#7fa0b8;animation:pulse .7s ease-in-out infinite alternate}#activity.error{background:#d65b68}@keyframes pulse{to{opacity:.25}}</style></head>
<body><header><h1>ELECTRIC SKY · CAMERA</h1><div><span id="activity" title="idle"></span><button id="capture">capture</button> <label>auto <select id="interval"><option value="0">off</option><option value="1000">1s</option><option value="2000">2s</option><option value="5000">5s</option></select></label></div></header><main><img id="image" alt="Camera snapshot"><p id="state">last image —</p></main>
<script>const image=document.getElementById('image'),state=document.getElementById('state'),activity=document.getElementById('activity'),interval=document.getElementById('interval');let timer=null;function capture(){activity.className='busy';activity.title='capturing';image.src='/camera.jpg?t='+Date.now()}image.onload=()=>{activity.className='';activity.title='idle';state.textContent='last image '+new Date().toLocaleTimeString()};image.onerror=()=>{activity.className='error';activity.title='capture failed'};document.getElementById('capture').onclick=capture;interval.onchange=()=>{clearInterval(timer);timer=null;if(+interval.value){capture();timer=setInterval(capture,+interval.value)}};capture();</script></body></html>)HTML";
    server.sendHeader("Connection", "close");
    server.send(200, "text/html", page);
    server.client().stop();
  });
