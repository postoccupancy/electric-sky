...
<section class="scope"><div class="scope-head"><span class="name">Solar input power</span><span class="reading" id="solarValue">—</span></div><canvas id="solar"></canvas></section>
...
const rings={temp:new Ring(2500,10000),humidity:new Ring(2500,10000),pressure:new Ring(2500,10000),power:new Ring(25000,1000),solar:new Ring(25000,1000),audio:new Ring(6250,4000)};
...
let networkRssi=0,wifiReconnects=0,oscFailures=0,oscSendAvgUs=0,oscSendMaxUs=0,oscSendStalls=0;
let solarVolts=0,solarCurrent=0;
let viewEnd=null;
...
function parsePacket(buffer){
  ...
  networkRssi=d.getInt16(72,true);wifiReconnects=d.getUint16(74,true);oscFailures=d.getUint32(76,true);oscSendAvgUs=d.getUint32(80,true);oscSendMaxUs=d.getUint32(84,true);oscSendStalls=d.getUint32(88,true);
  ...
  const powerBytes=d.getUint8(4)>=2?36:24;
  for(let i=0;i<np;i++,o+=powerBytes){const seq=d.getUint32(o,true),t=u64(d,o+4);rings.power.push({seq,t,v:d.getFloat32(o+20,true)/1000});if(powerBytes>=36&&d.getUint8(5)&1){solarVolts=d.getFloat32(o+24,true);solarCurrent=d.getFloat32(o+28,true);rings.solar.push({seq,t,v:d.getFloat32(o+32,true)/1000})}}
...
function draw(name,color,unit,decimals,end,dt){
  ...
  const ring=rings[name],latest=ring.latest();if(!latest||end===null)return;const start=end-WINDOW_US,bins=Math.max(1,Math.ceil(w/2)),binUs=WINDOW_US/bins,firstBin=Math.floor(start/binUs),storageBins=bins+2;if(!canvas._mins||canvas._mins.length!==storageBins){canvas._mins=new Float32Array(storageBins);canvas._maxs=new Float32Array(storageBins)}const mins=canvas._mins,maxs=canvas._maxs;mins.fill(Infinity);maxs.fill(-Infinity);
  let lo=Infinity,hi=-Infinity;ring.visitRange(start,end,s=>{if(s.v<lo)lo=s.v;if(s.v>hi)hi=s.v;const b=Math.floor(s.t/binUs)-firstBin;if(b>=0&&b<storageBins){if(s.v<mins[b])mins[b]=s.v;if(s.v>maxs[b])maxs[b]=s.v}});if(lo===Infinity)return;const dataLo=lo,dataHi=hi,peakToPeak=dataHi-dataLo;let range=peakToPeak;if(range<1e-9)range=1;const wantedLo=dataLo-range*.12,wantedHi=dataHi+range*.12;if(canvas._axisLo===undefined){canvas._axisLo=wantedLo;canvas._axisHi=wantedHi}else{if(wantedLo<canvas._axisLo)canvas._axisLo=wantedLo;if(wantedHi>canvas._axisHi)canvas._axisHi=wantedHi}lo=canvas._axisLo;hi=canvas._axisHi;
  c.strokeStyle='#1b252d';c.lineWidth=scale;c.beginPath();for(let i=1;i<4;i++){const y=h*i/4;c.moveTo(0,y);c.lineTo(w,y)}c.stroke();
  c.strokeStyle=color;c.lineWidth=2*scale;c.beginPath();let previous=-2;for(let b=0;b<storageBins;b++){if(mins[b]===Infinity)continue;const binTime=(firstBin+b+.5)*binUs,x=(binTime-start)/WINDOW_US*w,y1=h-(mins[b]-lo)/(hi-lo)*h,y2=h-(maxs[b]-lo)/(hi-lo)*h;if(b!==previous+1)c.moveTo(x,y1);else c.lineTo(x,y1);if(y2!==y1)c.lineTo(x,y2);previous=b}c.stroke();
  ...
  document.getElementById(name+'Value').textContent=name==='solar'?latest.v.toFixed(decimals)+' W · '+solarVolts.toFixed(3)+' V · '+solarCurrent.toFixed(1)+' mA':latest.v.toFixed(decimals)+' '+unit;
}
let lastPaint=0,paintAccumulator=0;
function render(now){const newest=newestTime(),dt=lastFrame?now-lastFrame:0;if(lastFrame){if(dt>40)renderStalls++;if(dt>worstFrame)worstFrame=dt}if(viewEnd===null&&newest)viewEnd=newest-presentationDelayUs;else if(lastFrame&&viewEnd!==null)viewEnd+=dt*1000;const underrun=viewEnd!==null&&viewEnd>newest;if(underrun&&!inUnderrun){displayUnderruns++;underrunStarted=now}else if(!underrun&&inUnderrun){underrunMs+=now-underrunStarted;underrunStarted=0}inUnderrun=underrun;lastFrame=now;paintAccumulator+=dt;if(!lastPaint||paintAccumulator>=1000/60){const paintDt=lastPaint?now-lastPaint:0;paintAccumulator%=1000/60;lastPaint=now;draw('temp','#66ddff','°C',4,viewEnd,paintDt);draw('humidity','#75ee99','%',4,viewEnd,paintDt);draw('pressure','#dd99ff','hPa',4,viewEnd,paintDt);draw('power','#ffcc66','W',4,viewEnd,paintDt);draw('solar','#ff9f43','W',4,viewEnd,paintDt);draw('audio','#ff6688','dBFS',2,viewEnd,paintDt)}requestAnimationFrame(render)}
let prevPackets=0,prevBytes=0,prevPacketGaps=0,prevOscFailures=0,prevOscStalls=0;
setInterval(async()=>{
  const now=performance.now(),dp=packetCount-prevPackets,db=bytesReceived-prevBytes,observedSec=Math.max(.001,(now-observationStarted)/1000),avg=packetCount/observedSec,currentSilence=lastArrival?now-lastArrival:0,totalUnderrun=underrunMs+(inUnderrun?now-underrunStarted:0),dropDelta=baselineDrops===null?0:transportDropped-baselineDrops,dropSeconds=Math.max(.001,(firmwareUptimeMs-baselineUptimeMs)/1000),dropMin=dropDelta/(dropSeconds/60),recentGaps=packetGaps-prevPacketGaps,recentOscFailures=oscFailures-prevOscFailures,recentOscStalls=oscSendStalls-prevOscStalls;prevPackets=packetCount;prevBytes=bytesReceived;prevPacketGaps=packetGaps;prevOscFailures=oscFailures;prevOscStalls=oscSendStalls;
  ...
  const offline=currentSilence>2000,congested=!offline&&(recentGaps>0||recentOscFailures>0||recentOscStalls>0||oscSendAvgUs>20000),degraded=!offline&&!congested&&(jitterEma>50||networkRssi<-70||oscSendAvgUs>5000);networkHealth.textContent=offline?'offline':congested?'congested':degraded?'degraded':'healthy';networkHealth.className=offline||congested?'loss':degraded?'low':'healthy';networkDetail.textContent='RSSI '+networkRssi+' dBm · OSC send avg/max '+(oscSendAvgUs/1000).toFixed(1)+'/'+(oscSendMaxUs/1000).toFixed(1)+'ms · recent gaps/failures/stalls '+recentGaps+'/'+recentOscFailures+'/'+recentOscStalls+' · totals '+packetGaps+'/'+oscFailures+'/'+oscSendStalls+' · Wi-Fi reconnects '+wifiReconnects;
  jitter.textContent=jitterEma.toFixed(1)+' ms';browserDetail.textContent='silence now/max '+currentSilence.toFixed(0)+'/'+longestSilence.toFixed(0)+'ms · sample gaps B/P/S/A events '+rings.temp.gapEvents+'/'+rings.power.gapEvents+'/'+rings.solar.gapEvents+'/'+rings.audio.gapEvents+' missing '+rings.temp.gaps+'/'+rings.power.gaps+'/'+rings.solar.gaps+'/'+rings.audio.gaps+' duration '+(rings.temp.gapUs/1e6).toFixed(1)+'/'+(rings.power.gapUs/1e6).toFixed(1)+'/'+(rings.solar.gapUs/1e6).toFixed(1)+'/'+(rings.audio.gapUs/1e6).toFixed(1)+'s · underruns '+displayUnderruns+' for '+(totalUnderrun/1000).toFixed(1)+'s · render stalls '+renderStalls+' · worst '+worstFrame.toFixed(0)+'ms';
